...
#include <fstream>
//Переменные
int k = 0;//переменная которая получает ввод из меню
int n = 0;//переменная нужная для функции создания трубы
//Переменные(трубы)
...
//Функции
void menu() {
    std::cout << "///////  Меню  ///////" << std::endl << "1) Добавить трубу" << std::endl << "2) Добавить КС" << std::endl << "3) Просмотр всех объектов" << std::endl << "4) Редактировать трубу" << std::endl << "5) Редактировать КС" << std::endl << "6) Сохранить" << std::endl << "7) Загрузить" << std::endl << "0) Выход" << std::endl << "Введите команду которую вы бы хотели выполнить(от 0 до 7): ";
    (std::cin >> k).get();
